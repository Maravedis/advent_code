...
(defn part2 ([path] (let [elves (u/read-file-line path parse-long)] (part2 elves 1 elves 2)))
  ([n first-elf last-elf step]
   (cond (= n 2) first-elf
         (= n 3) last-elf
         :else (case (mod n 3)
                 0 (recur (quot n 3) (+ first-elf step) last-elf (if (= step 2) 6 (* step 3)))
                 1 (recur (inc (quot n 3)) first-elf (- last-elf step) (if (= step 2) 6 (* step 3)))
                 2 (recur (inc (quot n 3)) (+ first-elf (quot step 2)) (- last-elf (quot step 2)) (if (= step 2) 6 (* step 3)))))))
